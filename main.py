...
async def handle_angebot(request: Request):
    ...
    log.info("Angebot-Webhook empfangen: %s", body)
    ...
    if not email:
        raise HTTPException(status_code=422, detail="E-Mail fehlt im Formular")
    ...
    # Pipedrive: Person finden oder anlegen
    ...
    # Pipedrive: Deal anlegen (nur wenn noch kein offener Deal existiert)
    ...
    lx_deeplink = ""
    try:
        lx_contact_id = await lx.find_or_create_contact(
        ...
        lx_quote = await lx.create_quote(
            contact_id=lx_contact_id,
            line_items=lx_line_items,
            title=f"Angebot für {company or name}",
        )
